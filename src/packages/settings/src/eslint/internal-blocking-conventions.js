...
const API_FILENAME_PATTERN = /^([A-Z][A-Za-z0-9]*)\.(?:service|type|api|zod)$/;
const TOP_LEVEL_BARREL_IMPORT_PATTERN =
  /^@\/(components|context|hooks|layout|lib|page|store)\//;
...
function getApiResourceName(filename) {
  return toPascalCase(path.basename(path.dirname(filename)));
}

function isTrailingLineComment(comment, sourceCode) {
  ...
  return Boolean(beforeComment.trim());
}

function isValidApiFilename(filename, baseName) {
  if (['index', 'api-codegen', 'instance'].includes(baseName)) {
    return true;
  }

  const match = API_FILENAME_PATTERN.exec(baseName);

  return Boolean(match && match[1] === getApiResourceName(filename));
}
...
function isQueryKeyMember(node) {
  if (node && node.type === 'CallExpression') {
    return isQueryKeyMember(node.callee);
  }
  ...
  create(context) {
    ...
    function checkImportDeclaration(node) {
      ...
      if (
        ...
        TOP_LEVEL_BARREL_IMPORT_PATTERN.test(source)
      ) {
        report(
          node.source,
          '`@/components`, `@/lib` 같은 최상단 barrel의 하위 경로 import는 금지됩니다.',
        );
      }
    ...
    function checkFileNaming(programNode) {
      ...
      if (
        ...
        if (!isValidApiFilename(filename, baseName)) {
          const resourceName = getApiResourceName(filename);

          report(
            programNode,
            `\`${resourceName}\` 폴더의 API 파일명은 ${resourceName}.service.ts, ${resourceName}.type.ts, ${resourceName}.api.ts, ${resourceName}.zod.ts 중 하나여야 합니다.`,
          );
        }
      }
      ...
      const isModalContextComponent =
        /\/context\/Modal\/(?:Container|Modal)\.tsx?$/.test(filename);

      if (
        filename.includes('/context/') &&
        baseName !== 'index' &&
        !isModalContextComponent
      ) {
        ...
        if (isDefaultExportFunction(defaultExport)) {
          ...
          if (functionNode.params.length > 0) {
            if (functionNode.params.length !== 1) {
              report(
                defaultExport,
                '커스텀 훅 인자는 단일 props 객체로 받아야 합니다.',
              );
            } else {
              ...
              if (
                ...
                if (!hasPropsDestructuring(functionNode, propsParam.name)) {
                  report(
                    propsParam,
                    '커스텀 훅 props는 함수 내부에서 구조 분해해야 합니다.',
                  );
                }
                ...
                if (!propsDeclaration) {
                  report(
                    defaultExport,
                    '커스텀 훅 props 타입은 Props로 선언해야 합니다.',
                  );
                } else if (
                  getDeclarationKind(propsDeclaration) !==
                  'TSInterfaceDeclaration'
                ) {
                  report(
                    propsDeclaration,
                    '커스텀 훅 Props는 interface로 선언해야 합니다.',
                  );
                }
              }
            }
    ...
    return {
      Program(programNode) {
        ...
        for (const comment of comments) {
          ...
          if (isTrailingLineComment(comment, sourceCode)) {
            report(
              comment,
